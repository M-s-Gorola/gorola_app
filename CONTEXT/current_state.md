...
- **Blue Ring Fix**: Identified that an aggressive global `*` selector in `globals.css` was applying `outline-ring/50` to every element, causing conflicts with `focus:outline-none` and triggering browser-default blue rings in production. Replaced with a targeted `:focus-visible` rule. Per user request, converted this into a permanent "Saffron Halo" (`ring-2 ring-gorola-saffron`) around the profile button for a premium, bold orange look that matches the cart.
