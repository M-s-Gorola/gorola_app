...
describe("Order Payment Status Integration", () => {
  ...
  it("places order with paymentMethod COD and sets paymentStatus to PENDING", async () => {
    ...
    expect(envelope.data.paymentMethod).toBe("COD");
    expect(envelope.data.paymentStatus).toBe("PENDING");
    expect(envelope.data.razorpayOrderId).toBeNull();
    
    // Assert fields on order directly in DB
    const orderInDb = await db.order.findUniqueOrThrow({
      where: { id: envelope.data.id }
    });
    expect(orderInDb.paymentStatus).toBe("PENDING");
    expect(orderInDb.razorpayOrderId).toBeNull();
  });

  it("places order with paymentMethod UPI and sets paymentStatus to PENDING and razorpayOrderId to null", async () => {
    process.env.GOROLA_TEST_OTP = "111222";
    const server = createServer({
      disableRedis: true,
      registerRoutes: registerAppRoutes
    ...
    expect(envelope.success).toBe(true);
    expect(envelope.data.paymentMethod).toBe("UPI");
    expect(envelope.data.paymentStatus).toBe("PENDING");
    expect(envelope.data.razorpayOrderId).toBeNull();
