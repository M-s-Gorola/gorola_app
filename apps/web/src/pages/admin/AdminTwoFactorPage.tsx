...
export function AdminTwoFactorPage(): ReactElement {
  ...
  const onSubmit = handleSubmit(async (vals) => {
    ...
    setLoading(true);
    try {
      const res = await api.post<VerifyEnvelope>("/api/v1/auth/admin/login", {
        email,
        password,
        totpCode: vals.code
      });
    ...
    <div className="mx-auto flex min-h-[60vh] max-w-md flex-col justify-center gap-6 px-4 py-10">
      ...
      <form className="flex flex-col gap-4" onSubmit={onSubmit}>
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium leading-none" htmlFor="totp-code">
            Two-Factor Code
          </label>
          <Input
            id="totp-code"
