...
export function StoreSetup2FAPage(): ReactElement {
  ...
  useEffect(() => {
    ...
    const initSetup = async () => {
      setLoading(true);
      try {
        const res = await httpClient.post<SetupResponse>("/api/v1/auth/store-owner/setup-2fa", {
          email
        });
        const body = res.data;
  ...
  const onSubmit = handleSubmit(async (vals) => {
    ...
    setLoading(true);
    try {
      const res = await httpClient.post<VerifyResponse>("/api/v1/auth/store-owner/verify-2fa", {
        email,
        code: vals.code
      });

      const body = res.data;
      if (body.success === true && body.data?.verified === true) {
    ...
    <div className="mx-auto flex min-h-[60vh] max-w-md flex-col justify-center gap-6 px-4 py-10">
      ...
      {errorMessage && !setupData && (
        <div className="flex flex-col gap-4 items-center">
          ...
          <Button className="rounded-full" onClick={() => navigate("/store/login")}>
            Back to Login
          </Button>
        </div>
      )}
