...
type StoreLoginFormValues = z.infer<typeof storeLoginSchema>;
...
export function StoreLoginPage(): ReactElement {
  ...
  const onSubmit = handleSubmit(async (vals) => {
    ...
    setLoading(true);
    try {
      const res = await api.post<LoginEnvelope>("/api/v1/auth/store-owner/login", {
        email: vals.email,
  ...
  return (
    <div className="mx-auto flex min-h-[60vh] max-w-md flex-col justify-center gap-6 px-4 py-10">
      <div className="flex flex-col gap-2 text-center">
        <h1 className="font-heading text-3xl tracking-tight text-gorola-charcoal">Store Partner Portal</h1>
        <p className="text-muted-foreground text-sm">Sign in to manage your store location</p>
      </div>

      <form className="flex flex-col gap-4" onSubmit={onSubmit}>
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium leading-none" htmlFor="store-login-email">
            Email address
          </label>
          <Input
